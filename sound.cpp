...
//--------------------------------------------------------------------------------------------
// 周波数変更 再生速度が変わる
// se::SetFrequency("testse1", se::RATE_SET::NORMAL);
// se::RATE_SET::NORMAL　通常の速さに戻る
// se::RATE_SET::DOUBLE	 呼び出す毎に2倍早くなる。
// se::RATE_SET::HALF	 呼び出す毎に半分の速さになる。
// 限界まで遅くなるor早くなるともう変更できない。
//--------------------------------------------------------------------------------------------
void se::SetFrequency(const std::string& resname, se::RATE_SET f) {
	auto seTemp = SeTable.at(resname).se;
	switch (f) {
	case se::RATE_SET::NORMAL:
		se::SetFrequency(resname, DSBFREQUENCY_ORIGINAL);
		break;
	case se::RATE_SET::DOUBLE:
		se::SetFrequency(resname, seTemp.Fre);
		break;
	case se::RATE_SET::HALF:
		se::SetFrequency(resname, -(int)seTemp.Fre/2);
		break;
	}
}
...
//--------------------------------------------------------------------------------------------
// 周波数変更 再生速度が変わる
// se::SetFrequency("testse1", se::RATE_CONTROL::NORMAL);
// se::RATE_CONTROL::NORMAL　通常の速さに戻る
// se::RATE_CONTROL::FASTER	 呼び出す毎に少しだけ早くなる。
// se::RATE_CONTROL::LATER	 呼び出す毎に少しだけ遅くなる。
// 限界まで遅くなるor早くなるともう変更できない。
//--------------------------------------------------------------------------------------------
void se::SetFrequency(const std::string& resname, se::RATE_CONTROL f)
{
	auto seTemp = SeTable.at(resname).se;
	switch (f) {
	case se::RATE_CONTROL::NORMAL:
		se::SetFrequency(resname, DSBFREQUENCY_ORIGINAL);
		break;
	case se::RATE_CONTROL::FASTER:
		se::SetFrequency(resname, +1000);
		break;
	case se::RATE_CONTROL::LATER:
		se::SetFrequency(resname, -1000);
		break;
	}
}
...
// 通常利用しない
//--------------------------------------------------------------------------------------------
DWORD se::SetFrequency(const std::string& resname, int Fre)
{
	auto& seTemp = SeTable.at(resname).se;
	if (seTemp.Fre + Fre < DSBFREQUENCY_MIN) {
		seTemp.sBuffer->SetFrequency(DSBFREQUENCY_MIN);
		for (auto& s : seTemp.PlayBuffer)
		{
			s.pBuffer->SetFrequency(DSBFREQUENCY_MIN);
		}

	}
	else if (seTemp.Fre + Fre > DSBFREQUENCY_MAX) {
		seTemp.sBuffer->SetFrequency(DSBFREQUENCY_MAX);
		for (auto& s : seTemp.PlayBuffer)
		{
			s.pBuffer->SetFrequency(DSBFREQUENCY_MAX);
		}
	}
	else {
		seTemp.sBuffer->SetFrequency(DWORD(seTemp.Fre + Fre));
		for (auto& s : seTemp.PlayBuffer)
		{
			s.pBuffer->SetFrequency(DWORD(seTemp.Fre + Fre));
		}
	}
	DWORD newFre;
	seTemp.sBuffer->GetFrequency(&newFre);
	seTemp.Fre = seTemp.Fre + Fre;
	return newFre;
}

long se::SetVolume(const std::string& resname, int Vol)
{
	long volume;
